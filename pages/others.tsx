...
const othersImages: OthersImage[] = [
  { src: '/others-1.jpeg', label: 'God of Fortune' },
  { src: '/others-2.jpeg', label: 'Mascot' },
  { src: '/others-3.jpeg', label: 'Chinese Orchestra' },
  { src: '/others-4.jpeg', label: 'Face-Changing' },
  { src: '/others-5.jpeg', label: 'Dance' },
  { src: '/others-6.jpeg', label: 'Acrobatics' },
  { src: '/others-7.jpeg', label: 'Wushu' },
  { src: '/others-8.jpeg', label: 'Calligraphy' },
];
...
export default function OthersPage() {
  ...
  const goToNext = () => {
    ...
    setCurrentIndex((prev) => (prev + 1) % othersImages.length);
    setTimeout(() => setJustChangedIndex(false), 50);
  };

  const goToPrev = () => {
    setJustChangedIndex(true);
    setCurrentIndex((prev) => (prev - 1 + othersImages.length) % othersImages.length);
    setTimeout(() => setJustChangedIndex(false), 50);
  };
  ...
  const prevIndex = (currentIndex - 1 + othersImages.length) % othersImages.length;
  const nextIndex = (currentIndex + 1) % othersImages.length;
  const nextNextIndex = (currentIndex + 2) % othersImages.length;
  const prevPrevIndex = (currentIndex - 2 + othersImages.length) % othersImages.length;
  ...
  return (
    <>
      <Head>
        <title>Others Gallery - Kwong Ngai Lion Dance</title>
        <meta name="description" content="Gallery showcasing other magnificent lion dance performances" />
      </Head>
            ...
            {/* Far Previous (i-2) - Appears when dragging left */}
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none" style={{ opacity: Math.max(0, Math.min(0.35, -dragProgress - 0.3)), transform: `translateX(${dragProgress * 50 - 150}%) scale(0.45)`, transition: (isDragging || justChangedIndex) ? 'none' : 'all 0.5s cubic-bezier(0.4, 0, 0.2, 1)', zIndex: 0 }}>
                <div className="relative w-[65%] h-[65%] max-w-[750px] rounded-2xl overflow-hidden shadow-2xl">
                  <Image src={othersImages[prevPrevIndex].src} alt="Far Previous" fill className="object-cover" sizes="(max-width: 768px) 100vw, 60vw" draggable={false} />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent">
                    <div className="absolute bottom-6 left-6 right-6">
                      <h3 className="text-xl md:text-2xl font-bold text-white" style={{ fontFamily: "'Alfa Slab One', cursive" }}>{othersImages[prevPrevIndex].label}</h3>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="absolute inset-0 flex items-center justify-center" style={{ opacity: dragProgress > 0 ? 0.55 + Math.min(1, dragProgress) * 0.45 : 0.55 - Math.min(0.3, -dragProgress * 0.3), transform: `translateX(${dragProgress > 0 ? dragProgress * 70 - 72 : -72}%) scale(${0.65 + (dragProgress > 0 ? Math.min(1, dragProgress) * 0.35 : -Math.min(0.1, -dragProgress * 0.1))})`, transition: (isDragging || justChangedIndex) ? 'none' : 'all 0.5s cubic-bezier(0.4, 0, 0.2, 1)', zIndex: 5 }}>
                <div className="relative w-[78%] h-[78%] max-w-[880px] rounded-2xl overflow-hidden shadow-2xl">
                  <Image src={othersImages[prevIndex].src} alt="Previous" fill className="object-cover" sizes="(max-width: 768px) 100vw, 65vw" draggable={false} />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent">
                    <div className="absolute bottom-7 left-7 right-7">
                      <h3 className="text-2xl md:text-3xl font-bold text-white" style={{ fontFamily: "'Alfa Slab One', cursive" }}>{othersImages[prevIndex].label}</h3>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="absolute inset-0 flex items-center justify-center" style={{ opacity: getCurrentOpacity(), transform: getCurrentTransform(), transition: (isDragging || justChangedIndex) ? 'none' : 'all 0.5s cubic-bezier(0.4, 0, 0.2, 1)', zIndex: 10 }}>
                <div className="relative w-[85%] h-[85%] max-w-[950px] rounded-2xl overflow-hidden shadow-2xl">
                  <Image src={othersImages[currentIndex].src} alt={othersImages[currentIndex].label} fill className="object-cover" sizes="(max-width: 768px) 100vw, 75vw" priority draggable={false} />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent">
                    <div className="absolute bottom-8 left-8 right-8">
                      <h3 className="text-3xl md:text-4xl font-bold text-white" style={{ fontFamily: "'Alfa Slab One', cursive" }}>{othersImages[currentIndex].label}</h3>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="absolute inset-0 flex items-center justify-center" style={{ opacity: dragProgress < 0 ? 0.55 + Math.min(1, -dragProgress) * 0.45 : 0.55 - Math.min(0.3, dragProgress * 0.3), transform: `translateX(${dragProgress < 0 ? dragProgress * 70 + 72 : 72}%) scale(${0.65 + (dragProgress < 0 ? Math.min(1, -dragProgress) * 0.35 : -Math.min(0.1, dragProgress * 0.1))})`, transition: (isDragging || justChangedIndex) ? 'none' : 'all 0.5s cubic-bezier(0.4, 0, 0.2, 1)', zIndex: 5 }}>
                <div className="relative w-[78%] h-[78%] max-w-[880px] rounded-2xl overflow-hidden shadow-2xl">
                  <Image src={othersImages[nextIndex].src} alt="Next" fill className="object-cover" sizes="(max-width: 768px) 100vw, 65vw" draggable={false} />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent">
                    <div className="absolute bottom-7 left-7 right-7">
                      <h3 className="text-2xl md:text-3xl font-bold text-white" style={{ fontFamily: "'Alfa Slab One', cursive" }}>{othersImages[nextIndex].label}</h3>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none" style={{ opacity: Math.max(0, Math.min(0.35, -dragProgress - 0.3)), transform: `translateX(${dragProgress * 50 + 150}%) scale(0.45)`, transition: (isDragging || justChangedIndex) ? 'none' : 'all 0.5s cubic-bezier(0.4, 0, 0.2, 1)', zIndex: 0 }}>
                <div className="relative w-[65%] h-[65%] max-w-[750px] rounded-2xl overflow-hidden shadow-2xl">
                  <Image src={othersImages[nextNextIndex].src} alt="Far Next" fill className="object-cover" sizes="(max-width: 768px) 100vw, 60vw" draggable={false} />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent">
                    <div className="absolute bottom-6 left-6 right-6">
                      <h3 className="text-xl md:text-2xl font-bold text-white" style={{ fontFamily: "'Alfa Slab One', cursive" }}>{othersImages[nextNextIndex].label}</h3>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-[200] bg-black/95 flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setIsModalOpen(false)}>
          ...
          <div className="relative max-w-6xl w-full" onClick={(e) => e.stopPropagation()}>
            <div className="relative aspect-[16/9] w-full">
              <Image src={othersImages[currentIndex].src} alt={othersImages[currentIndex].label} fill className="object-contain rounded-lg" sizes="(max-width: 640px) 100vw, 90vw" priority />
            </div>
            <div className="text-center mt-6">
              <h3 className="text-4xl font-bold text-white" style={{ fontFamily: "'Alfa Slab One', cursive" }}>{othersImages[currentIndex].label}</h3>
            </div>
          </div>
